...
interface AppointmentsByTopicChartProps {
  selectedMonth: string;
  churchId: string | null;
}

export function AppointmentsByTopicChart({ selectedMonth, churchId }: AppointmentsByTopicChartProps) {
  ...
  useEffect(() => {
    ...
    fetchChartData();
  }, [selectedMonth, churchId, toast]);

  return (
    <Card>
        <CardHeader>
            <CardTitle>Necessidades Atendidas</CardTitle>
            <CardDescription>Distribuição de atendimentos por tópico solicitado no período.</CardDescription>
        </CardHeader>
        <CardContent>
            {loading ? (
                ...
                <div className="flex items-center justify-center h-[250px] text-center text-sm text-muted-foreground">
                    Nenhum tópico de atendimento encontrado para exibir o gráfico no período selecionado.
                </div>
            )}
      </CardContent>
    </Card>
